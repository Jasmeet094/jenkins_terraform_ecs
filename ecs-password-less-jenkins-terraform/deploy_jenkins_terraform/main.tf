...
module "service" {
  ...
  health_http_check_port = var.services.aws_test.health_check_port
  health_check_path      = var.services.aws_test.health_check
  ...
  container_port         = var.services.aws_test.port_mappings[0].containerPort
  desired_count          = var.services.aws_test.desired
  cluster                = module.ecs_cluster.cluster_name
  subnets                = module.network.private_subnet_ids
  scale_threshold        = var.services.aws_test.scale_threshold
  min_task_capacity      = var.services.aws_test.min_task_capacity
  max_task_capacity      = var.services.aws_test.max_task_capacity
  tags                   = merge(var.tags, { service_name = "aws_test" })
...
module "jenkins_controller_task" {
  ...
  fargate_cpu                  = var.services.aws_test.cpu
  fargate_memory               = var.services.aws_test.memory
  container_image              = module.ecr.repository_url
  container_memory_reservation = var.services.aws_test.memory
  file_system_id               = module.efs.efs_volume_id
  secrets                      = var.services.aws_test.secrets
...
module "task_role_worker" {
  ...
  ], var.services.aws_test.iam_policies)
  ...
  identifier            = "${var.identifier}-aws_test-task"
  aws_account_id        = var.aws_account_id
  region                = var.aws_region
  tags                  = var.tags
}
