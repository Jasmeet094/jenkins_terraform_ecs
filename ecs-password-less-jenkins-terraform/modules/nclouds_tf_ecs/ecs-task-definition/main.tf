...
NOTE: Use the value of jenkins_url in "local.jenkins_controller_container_def" as same as the value you are using
for the Route53 record name in aws_route53_record resource in "deploy_jenkins_terraform/main.tf" 
...
locals {
  ...
  jenkins_controller_container_def = templatefile("${path.module}/../../templates/jenkins-controller.json.tftpl", {
    ...
    jenkins_url             = "https://jenkins-mig.tronixtrm.com/"
  })
}

resource "aws_ecs_task_definition" "jenkins_controller" {
  family                   = "${var.identifier}-${terraform.workspace}-tb"
